...
export function demostrarMatrices(): void {
  const m = crear3x3();
  console.log('Matriz 3x3:');
  imprimirTabla(m);
  console.log('Recorrido por columnas:');
  recorrerPorColumnas(m);
  console.log('Suma de elementos:', sumarElementos(m));
  intercambiarPrimeraUltimaFila(m);
  console.log('Matriz con primera y ultima fila intercambiadas:');
  imprimirTabla(m);
  console.log('Matriz irregular:');
  imprimirTabla([[1, 2], [3, 4, 5], [6]]);
}
